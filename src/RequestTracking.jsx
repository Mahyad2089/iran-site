...
const STEPS = [
  { title: "ثبت درخواست", meta: "۱۴۰۵/۰۴/۲۸ - ۱۰:۲۰", state: "done" },
  { title: "بررسی کارشناسی", meta: "۱۴۰۵/۰۴/۳۰ - در حال انجام", state: "active" },
  { title: "تأیید نهایی و صدور", meta: "در انتظار", state: "pending" },
];
...
export default function RequestTracking() {
  ...
  return (
    <ServiceLayout
      ...
      description="با استفاده از کد پیگیری، وضعیت درخواست‌های ثبت‌شده خود را در هر مرحله مشاهده کنید."
      crumbs={["خدمات شهروندی", "پیگیری درخواست‌ها"]}
    >
      {(step === "idle" || step === "loading") && (
        <>
          <form className="form-row" onSubmit={track}>
            <div className="field">
              <label>کد پیگیری</label>
              <input type="text" placeholder="مثلاً پ-۷۷۲۲۱۴" value={code} onChange={(e) => setCode(e.target.value)} disabled={step === "loading"} />
            </div>
            <button type="submit" className="btn-dark" style={{ alignSelf: "flex-end" }} disabled={step === "loading"}>
              {step === "loading" ? "در حال بررسی..." : "پیگیری"}
            </button>
          </form>
          {step === "loading" && <div className="loading-row"><Spinner />در حال دریافت وضعیت درخواست...</div>}
        </>
      )}
      ...
      {step === "result" && (
        <>
          <div className="result-head">
            <h2>وضعیت درخواست {code}</h2>
            <span className="badge warn">در حال بررسی</span>
          </div>
          <ul className="timeline">
            {STEPS.map((s) => (
              <li className={s.state} key={s.title}>
