...
const HISTORY = [
  { title: "صدور پایان‌کار ساختمانی", meta: "۱۴۰۱/۰۴/۱۲ · تأیید شده" },
  { title: "بازدید کارشناسی نما", meta: "۱۴۰۲/۰۲/۰۳ · بدون مغایرت" },
  { title: "پروانه ساختمانی اولیه", meta: "۱۳۹۹/۰۹/۲۱ · صادر شده" },
];

export default function PropertyRecordHistory() {
  ...
  return (
    <ServiceLayout
      eyebrow="استعلام‌ها"
      title="مشاهده سوابق پرونده ملک"
      description="سابقه‌ی مجوزها، بازدیدها و اقدامات ثبت‌شده برای پرونده‌ی ملک خود را مشاهده کنید."
      crumbs={["استعلام‌ها", "سوابق پرونده ملک"]}
    >
        ...
        <>
          <form className="form-row" onSubmit={inquire}>
            ...
            <button type="submit" className="btn-dark" style={{ alignSelf: "flex-end" }} disabled={step === "loading"}>
              {step === "loading" ? "در حال بررسی..." : "مشاهده سوابق"}
            </button>
          </form>
          {step === "loading" && <div className="loading-row"><Spinner />در حال بازیابی سوابق پرونده...</div>}
        </>
      )}
      ...
      {step === "result" && (
        <>
          <div className="result-head">
            <h2>سوابق ثبت‌شده</h2>
            <span className="badge success">{HISTORY.length} مورد</span>
          </div>
          <ul className="timeline">
            {HISTORY.map((h) => (
              <li className="done" key={h.title}>
                <div className="t-title">{h.title}</div>
                <div className="t-meta">{h.meta}</div>
              </li>
            ))}
          </ul>
