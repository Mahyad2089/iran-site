...
export default function CommissionVerdict() {
  ...
  return (
    <ServiceLayout
      eyebrow="استعلام‌ها"
      title="استعلام آراء کمیسیون ماده صد"
      description="نتیجه رأی صادر شده برای پرونده‌ی خود در کمیسیون ماده صد را پیگیری کنید."
      crumbs={["استعلام‌ها", "آراء کمیسیون ماده صد"]}
    >
      {(step === "idle" || step === "loading") && (
        <>
          <form className="form-row" onSubmit={inquire}>
            <div className="field">
              <label>شماره پرونده کمیسیون</label>
              <input type="text" placeholder="مثلاً ک۱۰۰-۹۹۸۸" value={code} onChange={(e) => setCode(e.target.value)} disabled={step === "loading"} />
            </div>
            <button type="submit" className="btn-dark" style={{ alignSelf: "flex-end" }} disabled={step === "loading"}>
              {step === "loading" ? "در حال بررسی..." : "استعلام رأی"}
            </button>
          </form>
          {step === "loading" && <div className="loading-row"><Spinner />در حال دریافت رأی از دبیرخانه کمیسیون...</div>}
        </>
      )}
      ...
      {step === "result" && (
        <>
          <div className="result-head">
            <h2>نتیجه رأی</h2>
            <span className="badge warn">رأی به جریمه نقدی</span>
          </div>
          <div className="info-grid">
            <div className="info-item"><span>شماره پرونده</span><b>{code}</b></div>
            <div className="info-item"><span>تاریخ رأی</span><b>۱۴۰۵/۰۳/۱۸</b></div>
            <div className="info-item full"><span>موضوع</span><b>تخلف ساختمانی - افزایش سطح اشغال مجاز</b></div>
            <div className="info-item full"><span>توضیحات رأی</span><b style={{ fontWeight: 500, lineHeight: 1.8 }}>پرونده در جلسه کمیسیون بدوی بررسی و با توجه به عدم مغایرت اساسی، به پرداخت جریمه نقدی رأی صادر شد.</b></div>
          </div>
          <div className="hint" style={{ textAlign: "center" }}>
            برای استعلام پرونده دیگر، <button
