...
export default function CertificateVerification() {
  ...
  return (
    <ServiceLayout
      eyebrow="استعلام‌ها"
      title="تأیید اعتبار گواهی شهرداری"
      description="صحت و اعتبار گواهی صادر شده توسط شهرداری را با شماره سریال آن استعلام بگیرید."
      crumbs={["استعلام‌ها", "تأیید اعتبار گواهی"]}
    >
      {(step === "idle" || step === "loading") && (
        <>
          <form className="form-row" onSubmit={inquire}>
            <div className="field">
              <label>شماره سریال گواهی</label>
              <input type="text" placeholder="مثلاً گ-۷۷۲۲۱۴" value={code} onChange={(e) => setCode(e.target.value)} disabled={step === "loading"} />
            </div>
            <button type="submit" className="btn-dark" style={{ alignSelf: "flex-end" }} disabled={step === "loading"}>
              {step === "loading" ? "در حال بررسی..." : "بررسی اعتبار"}
            </button>
          </form>
          {step === "loading" && <div className="loading-row"><Spinner />در حال تطبیق با سامانه صدور گواهی...</div>}
        </>
      )}
      ...
      {step === "result" && (
        <>
          <div className="result-head">
            <h2>نتیجه بررسی</h2>
            <span className={"badge " + (valid ? "success" : "danger")}>{valid ? "معتبر" : "نامعتبر"}</span>
          </div>
          {valid ? (
            <div className="info-grid">
              <div className="info-item"><span>شماره سریال</span><b>{code}</b></div>
              <div className="info-item"><span>تاریخ صدور</span><b>۱۴۰۴/۱۱/۰۲</b></div>
              <div className="info-item full"><span>نوع گواهی</span><b>گواهی عدم خلاف ساختمانی</b></div>
            </div>
          ) : (
            <p style={{ fontSize: 13.5, color: "var(--ink-600)", lineHeight: 1.8, margin: "0 0 8px" }}>
              این شماره سریال در سامانه صدور گواهی شهرداری یافت نشد. لطفاً از صحت شماره وارد شده اطمینان حاصل کنید یا با اداره صدور گواهی تماس بگیرید.
            </p>
          )}
          <div className="hint" style={{ textAlign: "center", marginTop: 12 }}>
            برای بررسی گواهی دیگر، <button
