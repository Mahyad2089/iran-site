...
export default function RenovationCodeLookup() {
  const [step, setStep] = useState("idle");
  const [mode, setMode] = useState("address"); // address | national
  const [query, setQuery] = useState("");
  const [copied, setCopied] = useState(false);
  const foundCode = "۱۲۴۴۸۸۷۷";
  ...
  return (
    <ServiceLayout
      eyebrow="استعلام‌ها"
      title="شناسایی کد نوسازی ملک"
      description="کد نوسازی ملک را بر اساس آدرس یا کد ملی مالک استعلام بگیرید."
      crumbs={["استعلام‌ها", "شناسایی کد نوسازی"]}
    >
      {(step === "idle" || step === "loading") && (
        <>
          <div className="field">
            <label>روش جست‌وجو</label>
            <div className="choice-group">
              <label className={"choice-card" + (mode === "address" ? " active" : "")}>
                <input type="radio" name="mode" checked={mode === "address"} onChange={() => setMode("address")} />
                بر اساس آدرس ملک
              </label>
              <label className={"choice-card" + (mode === "national" ? " active" : "")}>
                <input type="radio" name="mode" checked={mode === "national"} onChange={() => setMode("national")} />
                بر اساس کد ملی مالک
              </label>
            </div>
          </div>
          <form className="form-row" onSubmit={search}>
            <div className="field">
              <label>{mode === "address" ? "آدرس کامل ملک" : "کد ملی مالک"}</label>
              <input
                type="text"
                placeholder={mode === "address" ? "شهرآفتاب، فاز ۲، خیابان یاس، پلاک ۱۴" : "مثلاً ۰۰۱۲۳۴۵۶۷۸"}
                value={query}
            ...
            <button type="submit" className="btn-dark" style={{ alignSelf: "flex-end" }} disabled={step === "loading"}>
              {step === "loading" ? "در حال جست‌وجو..." : "جست‌وجو"}
            </button>
          </form>
          {step === "loading" && <div className="loading-row"><Spinner />در حال جست‌وجو در سامانه املاک...</div>}
        </>
      )}
      ...
      {step === "result" && (
        <>
          <div className="result-head">
            <h2>نتیجه استعلام</h2>
            <span className="badge success">یافت شد</span>
          </div>
          <div className="info-grid">
            <div className="info-item full"><span>آدرس ملک</span><b>شهرآفتاب، فاز ۲، خیابان یاس، پلاک ۱۴</b></div>
            <div className="info-item full"><span>کد نوسازی</span><b style={{ fontSize: 18 }}>{foundCode}</b></div>
          </div>
          <button className="btn-primary-block" onClick={copy}>
            {copied ? "کپی شد ✓" : "کپی کد نوسازی"}
          </button>
          <div className="hint" style={{ textAlign: "center", marginTop: 14 }}>
            برای استعلام ملک دیگر، <button
              onClick={() => { setStep("idle"); setQuery(""); }}
              style={{ background: "none", border: "none", color: "var(--blue-500)", fontWeight: 700, cursor: "pointer", padding: 0, fontFamily: "inherit" }}
            >اینجا کلیک کنید</button>.
          </div>
        </>
      )}
    </ServiceLayout>
