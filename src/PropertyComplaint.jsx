...
const SUBJECTS = ["اعتراض به مبلغ عوارض", "اعتراض به متراژ ثبت‌شده", "اعتراض به کاربری ملک"];

export default function PropertyComplaint() {
  const [step, setStep] = useState("idle");
  const [subject, setSubject] = useState(SUBJECTS[0]);
  const [code, setCode] = useState("");
  const [notes, setNotes] = useState("");
  const trackingCode = "اع-" + Math.floor(100000 + Math.random() * 899999);
  ...
  return (
    <ServiceLayout
      ...
      description="در صورت مغایرت اطلاعات ممیزی ملک با واقعیت، اعتراض خود را در سامانه ثبت کنید تا در کمیسیون مربوطه بررسی شود."
      crumbs={["خدمات شهروندی", "ثبت اعتراض ملکی"]}
    >
      {step !== "done" ? (
        <form onSubmit={submit}>
          <div className="field">
            <label>موضوع اعتراض</label>
          ...
          <div className="field">
            ...
            <textarea placeholder="دلیل اعتراض خود را با ذکر جزئیات شرح دهید..." value={notes} onChange={(e) => setNotes(e.target.value)} disabled={step === "loading"} />
          </div>
          <div className="field">
            <label>مستندات پیوست (اختیاری)</label>
            <div className="upload-box">برای بارگذاری تصاویر یا مدارک پشتیبان کلیک کنید</div>
          </div>
          <button type="submit" className="btn-primary-block" disabled={step === "loading"}>
            {step === "loading" ? (<><Spinner dark />در حال ثبت اعتراض...</>) : "ثبت اعتراض"}
          </button>
        </form>
      ) : (
        <div className="success-panel">
          ...
          <h2>اعتراض شما ثبت شد</h2>
          <p>پرونده اعتراض شما با موضوع «{subject}» ثبت و برای بررسی به کمیسیون ممیزی ارجاع شد.</p>
          <div className="tracking-code">کد پیگیری: {trackingCode}</div>
          <div className="success-actions">
            <button className="primary" onClick={() => { setStep("idle"); setCode(""); setNotes(""); }}>ثبت اعتراض جدید</button>
