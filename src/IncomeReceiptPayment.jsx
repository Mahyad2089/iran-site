...
const TYPES = ["عوارض متفرقه", "درآمد اجاره املاک", "سایر درآمدها"];

export default function IncomeReceiptPayment() {
  const [step, setStep] = useState("idle");
  const [type, setType] = useState(TYPES[0]);
  const [amount, setAmount] = useState("");
  const receiptNo = "ف-" + Math.floor(100000 + Math.random() * 899999);
  ...
  return (
    <ServiceLayout
      eyebrow="پرداخت‌ها"
      title="دریافت و پرداخت فیش درآمد"
      description="فیش درآمد شهرداری را برای نوع پرداخت مدنظر خود صادر و به‌صورت آنلاین پرداخت کنید."
      crumbs={["پرداخت‌ها", "فیش درآمد"]}
    >
      {(step === "idle" || step === "loading") && (
        <form onSubmit={generate}>
          <div className="field">
            <label>نوع فیش درآمد</label>
            <div className="choice-group">
              {TYPES.map((t) => (
                <label key={t} className={"choice-card" + (type === t ? " active" : "")}>
          ...
          <div className="field">
            <label>مبلغ فیش (ریال)</label>
            <input type="text" inputMode="numeric" placeholder="مثلاً ۵,۰۰۰,۰۰۰" value={amount} onChange={(e) => setAmount(e.target.value)} disabled={step === "loading"} />
          </div>
          <button type="submit" className="btn-dark" disabled={step === "loading"} style={{ width: "100%" }}>
            {step === "loading" ? "در حال صدور فیش..." : "صدور فیش"}
          </button>
          {step === "loading" && <div className="loading-row" style={{ marginTop: 8 }}><Spinner />در حال ثبت فیش در سامانه درآمد...</div>}
        </form>
      )}
      ...
      {(step === "result" || step === "paying") && (
        <>
          <div className="result-head">
            <h2>فیش صادر شده</h2>
            <span className="badge warn">در انتظار پرداخت</span>
          </div>
          <div className="info-grid">
            <div className="info-item"><span>شماره فیش</span><b>{receiptNo}</b></div>
            <div className="info-item"><span>نوع درآمد</span><b>{type}</b></div>
          </div>
          <div className="amount-row">
            <div>
              <div className="label">مبلغ قابل پرداخت</div>
              <div className="value">{amount}<span>ریال</span></div>
            </div>
          </div>
          <button className="btn-primary-block" onClick={pay} disabled={step === "paying"}>
            {step === "paying" ? (<><Spinner dark />در حال اتصال به درگاه پرداخت...</>) : "پرداخت آنلاین"}
          </button>
        </>
      )}
      ...
      {step === "paid" && (
        <div className="success-panel">
          ...
          <p>فیش شماره {receiptNo} به مبلغ {amount} ریال پرداخت شد.</p>
          <div className="success-actions">
            <button className="primary" onClick={() => { setStep("idle"); setAmount(""); }}>صدور فیش جدید</button>
