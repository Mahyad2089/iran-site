...
const CERT_TYPES = ["گواهی عدم خلاف", "گواهی پایان‌کار", "گواهی بلامانع نقل و انتقال"];

export default function IssueCertificateRequest() {
  const [step, setStep] = useState("idle");
  const [certType, setCertType] = useState(CERT_TYPES[0]);
  const [address, setAddress] = useState("");
  const [notes, setNotes] = useState("");
  const trackingCode = "پ-" + Math.floor(100000 + Math.random() * 899999);
  ...
  return (
    <ServiceLayout
      eyebrow="خدمات شهروندی"
      title="صدور گواهی شهرداری"
      description="درخواست صدور انواع گواهی‌های شهرداری را ثبت کنید تا پس از بررسی کارشناسان، نتیجه به شما اعلام شود."
      crumbs={["خدمات شهروندی", "صدور گواهی"]}
    >
      {step !== "done" ? (
        <form onSubmit={submit}>
          <div className="field">
            <label>نوع گواهی درخواستی</label>
            <select value={certType} onChange={(e) => setCertType(e.target.value)}>
              {CERT_TYPES.map((t) => <option key={t}>{t}</option>)}
            </select>
          </div>
          <div className="field">
            <label>آدرس کامل ملک</label>
            <input type="text" placeholder="شهرآفتاب، فاز ۲، خیابان یاس، پلاک ۱۴" value={address} onChange={(e) => setAddress(e.target.value)} disabled={step === "loading"} />
          </div>
          <div className="field">
            <label>مدارک پیوست (اختیاری)</label>
            <div className="upload-box">برای بارگذاری سند مالکیت یا مدارک تکمیلی کلیک کنید</div>
          </div>
          <div className="field">
            <label>توضیحات تکمیلی</label>
            <textarea placeholder="در صورت نیاز، توضیح مختصری درباره درخواست خود بنویسید..." value={notes} onChange={(e) => setNotes(e.target.value)} />
          </div>
          <button type="submit" className="btn-primary-block" disabled={step === "loading"}>
            {step === "loading" ? (<><Spinner dark />در حال ثبت درخواست...</>) : "ثبت درخواست"}
          </button>
        </form>
      ) : (
        <div className="success-panel">
          ...
          <p>درخواست «{certType}» با موفقیت ثبت شد. نتیجه بررسی طی چند روز کاری از طریق پیامک به شما اطلاع‌رسانی می‌شود.</p>
          <div className="tracking-code">کد پیگیری: {trackingCode}</div>
          <div className="success-actions">
            <button className="primary" onClick={() => { setStep("idle"); setAddress(""); setNotes(""); }}>ثبت درخواست جدید</button>
            <a className="ghost" href="#">پیگیری درخواست‌ها</a>
          </div>
        </div>
      )}
    </ServiceLayout>
