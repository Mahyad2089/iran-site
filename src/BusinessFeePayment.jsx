...
const MOCK = {
  code: "ص-۳۳۰۹۹۱",
  owner: "فروشگاه پارسیان",
  address: "شهرآفتاب، بلوار مرکزی، پلاک ۵۶",
  license: "پروانه کسب درجه ۲",
  amount: "۱۸,۲۰۰,۰۰۰",
  due: "۱۴۰۵/۰۷/۱۵",
};

export default function BusinessFeePayment() {
  ...
  return (
    <ServiceLayout
      ...
      description="با وارد کردن کد پرونده صنفی، مبلغ عوارض سالانه واحد کسبی خود را استعلام و پرداخت کنید."
      crumbs={["پرداخت‌ها", "عوارض کسب و پیشه"]}
    >
      {(step === "idle" || step === "loading") && (
        <>
          <form className="form-row" onSubmit={inquire}>
            <div className="field">
              <label>کد پرونده صنفی</label>
              <input type="text" placeholder="مثلاً ص-۳۳۰۹۹۱" value={code} onChange={(e) => setCode(e.target.value)} disabled={step === "loading"} />
            </div>
          ...
          {step === "loading" && (
            <div className="loading-row"><Spinner />در حال دریافت اطلاعات واحد صنفی...</div>
          )}
          {step === "idle" && <div className="hint">کد پرونده صنفی روی پروانه کسب یا قبض سال گذشته درج شده است.</div>}
        </>
      )}
      ...
      {(step === "result" || step === "paying") && (
        <>
          <div className="result-head">
            <h2>اطلاعات واحد صنفی</h2>
            <span className="badge warn">در انتظار پرداخت</span>
          </div>
          <div className="info-grid">
            <div className="info-item full"><span>آدرس واحد صنفی</span><b>{MOCK.address}</b></div>
            <div className="info-item"><span>نام واحد</span><b>{MOCK.owner}</b></div>
            <div className="info-item"><span>کد پرونده</span><b>{MOCK.code}</b></div>
            <div className="info-item full"><span>نوع پروانه</span><b>{MOCK.license}</b></div>
          </div>
          <div className="amount-row">
            <div>
              <div className="label">مبلغ قابل پرداخت · سررسید {MOCK.due}</div>
              <div className="value">{MOCK.amount}<span>ریال</span></div>
            </div>
          </div>
      ...
      {step === "paid" && (
        <div className="success-panel">
          ...
          <p>رسید پرداخت عوارض کسب و پیشه برای پرونده {MOCK.code} صادر شد.</p>
